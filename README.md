...
- **It self-heals.** Each data refresh probes the API once; the moment it answers, the real team table (and preferred-team ids) are reloaded, the notice stops and the indicator clears — no restart required. Recovery also pulls the NHL background workers forward immediately and recomputes the playoff/season phase: `standings`, `team_summary` and `stats_leaders` render from worker caches and `StandingsWorker` only refreshes hourly, so without that nudge those boards would sit on "unavailable" for up to an hour *after* the notice had already switched off. (The old crash loop got this for free by restarting; now that the process survives an outage, recovery has to do it explicitly.)
